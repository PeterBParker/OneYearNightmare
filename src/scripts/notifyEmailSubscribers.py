...
class EmailNotifier:
    ...
    def get_email_export_urls(self, pending_export_id):
        ...
        while export_pending:
            # check status
            response = self.sg.client.marketing.contacts.exports._(
                pending_export_id).get()
            if response.status_code != 200:
                print(
                    f"Error: Encountered {response.status_code} error in checking that status of the export.")
                print(response.body)
                handle_pending()
            else:
    ...
    def subscriber_emails(self):
        ...
        try:
            response = self.sg.client.marketing.contacts.exports.post(
                request_body={"list_ids": ["887040bc-71a4-489a-874a-c4cc0391f890"], "file_type": "csv"})
