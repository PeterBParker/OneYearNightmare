...
def addAnIconPathToDb(db_filename, icon_path, page_id):
    ...
    page["icon"] == str(Path(_ICON_REL_DIR) / icon_path.name)
    writeToFile(data, db_filename)
...
def addNewPage(seasonName, chapterName, title, message, filepath, user):
    ...
    season = getSeason(seasonName)
    destDir = Path(
        _DIR_PREFIX + season["folderName"] + "/" + chapter["folderName"] + "/"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            "Welcome to the Page Appender! I will append a page to the end of a specifed chapter.\n"
        )
